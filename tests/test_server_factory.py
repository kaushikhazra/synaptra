...
class TestGetEngineBackendBranching:
    ...
    def setup_method(self):
        ...
        import cognitive_memory.server as srv
        srv._engine = None
    ...
    def test_default_backend_is_surrealkv_file(self):
        ...
        with patch.dict(os.environ, env, clear=True):
            from cognitive_memory.server import _get_engine
            import cognitive_memory.server as srv
            srv._engine = None  # reset singleton

            mock_engine = MagicMock()
            with patch("cognitive_memory.server.MemoryEngine", return_value=mock_engine) as mock_me:
                engine = _get_engine()
    ...
    def test_surrealkv_file_explicit(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            srv._engine = None
            mock_engine = MagicMock()
            with patch("cognitive_memory.server.MemoryEngine", return_value=mock_engine):
    ...
    def test_rocksdb_server_creates_surreal_server_storage(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            ...
            with patch("cognitive_memory.surreal_server_storage.SurrealServerStorage",
    ...
    def test_rocksdb_server_default_url(self):
        ...
        with patch.dict(os.environ, env_clean, clear=True):
            ...
            with patch("cognitive_memory.surreal_server_storage.SurrealServerStorage") as mock_sss:
                mock_sss.return_value = MagicMock()
                with patch("cognitive_memory.server.MemoryEngine", return_value=MagicMock()):
                    srv._get_engine()
